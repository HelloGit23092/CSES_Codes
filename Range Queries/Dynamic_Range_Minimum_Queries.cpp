...
ll build(vll &segt, ll start, ll end, ll n, vll &arr)
{
	...
	ll mid = (start + end) / 2;
	segt[n] = min(build(segt, start, mid, 2 * n, arr), build(segt, mid + 1, end, 2 * n + 1, arr));
...
ll update(vll &segt, ll i, ll v, ll s, ll e, ll n)
{
	...
	ll mid = (s + e) / 2;
	segt[n] = min(update(segt, i, v, s, mid, 2 * n), update(segt, i, v, mid + 1, e, 2 * n + 1));
	return segt[n];
}
...
ll value(vll &segt, ll s, ll e, ll l, ll r, ll n)
{
	...
	ll mid = (s + e) / 2;
	return min(value(segt, s, mid, l, r, 2 * n), value(segt, mid + 1, e, l, r, 2 * n + 1));
}
