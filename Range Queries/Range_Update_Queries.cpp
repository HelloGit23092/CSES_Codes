...
void update(vll &lazySegt, ll s, ll e, ll l, ll r, ll v, ll n)
{
	...
	ll mid = (s + e) / 2;
	update(lazySegt,  s, mid, l, r, v, 2 * n);
	update(lazySegt, mid + 1, e, l, r, v, 2 * n + 1);
}
 
void query(vll &lazySegt, ll s, ll e, ll i, ll n, vll &arr)
{
	if (i < s || i > e)
		return;
	if (s == e)
	...
	lazySegt[2 * n] += lazySegt[n];
	lazySegt[2 * n + 1] += lazySegt[n];
	lazySegt[n] = 0;
	ll mid = (s + e) / 2;
	query(lazySegt, s, mid, i, 2 * n, arr);
	query(lazySegt, mid + 1, e, i, 2 * n + 1, arr);
}
	...
	while (T--)
	{
		...
		while (q--)
		{
			...
			if (t == 1)
			{
				...
				cin >> a >> b >> u;
				--a, --b;
				update(lazySegt, 0, n - 1, a, b, u, 1);
			}
			else
			{
				ll k;
				cin >> k;
				query(lazySegt, 0, n - 1, k - 1, 1, arr);
				cout << arr[k - 1] << "\n";
			}
		}
	}
